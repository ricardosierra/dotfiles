...
| **5. Modernização bootstrap (parte 1)** | **pip→pipx:** `30_python_pip.sh` + `60_python.sh` agora instalam tools isoladas via pipx. **nave removido:** `source/50_node.sh` perdeu 72 linhas (nave_install/nave_default/npm_globals fóssil); `init/50_node.sh` simplificado pra volta-only. **Brewfile criado:** `packages/Brewfile` consolidando 30_osx_homebrew_{casks,recipes}.sh (216 linhas) — .sh preservados como fallback até migração final | ✅ feito |
